	...
	public void flushChannelDetaile() {
		...
		for (TXZDownUserInfoPo txzDownUserInfoPo : list) {
			...
			try {
				...
				// 首页
				String fristContent = doGet("https://api.dsxzt.com/admin/access/v1/statistics/index?pageIndex=0&pageSize=10000&regChannel=ALL&startDate=&endDate=",
						sef);
				analyzeDataService.analyzeTFirstInfoPoList(fristContent);
				// 会员列表
				String mebersContent = doGet("https://api.dsxzt.com/admin/access/v1/statistics/userlist?pageIndex=1&pageSize=200&startDate=&endDate=&phone=&regChannel=ALL",
						sef);
				analyzeDataService.analyzeTRegistUserInfoPoList(mebersContent);
				
				
				// 历史数据
				String historyContent = doGet("https://api.dsxzt.com/admin/access/v1/statistics/history?pageIndex=1&pageSize=10000&startDate=&endDate=",
						sef);
